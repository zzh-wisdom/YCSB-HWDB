...
cmd="./ycsbc -db rocksdb -dbpath $dbpath -threads 4 -P $zipfian_workload -run true -morerun $moreworkloads -dbstatistics true >>out.out 2>&1 "

if [ -n "$1" ];then    #后台运行
cmd="nohup ./ycsbc -db rocksdb -dbpath $dbpath -threads 4 -P $zipfian_workload -run true -morerun $moreworkloads -dbstatistics true >>out.out 2>&1  &"
echo $cmd >out.out
fi
